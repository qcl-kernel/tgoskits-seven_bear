...
- **QEMU 与固件**：核验 QEMU 程序、机器类型、处理器、对称多处理数量、闪存或 OVMF 文件、串口、磁盘与根文件系统设备、`-snapshot`、调试参数、超时和成功或失败匹配规则。通过 `cargo xtask ovmf --arch <arch>` 取得 OVMF CODE 与 VARS；该命令复用 Ostool 固定版本、镜像探测、SHA-256 校验和 `$TMPDIR/ostool/ovmf` 缓存。`TGOS_OVMF_DIR` 只用于选择另一 Ostool 格式缓存根目录，不能给各消费者新增固件变量或扫描发行版 `/usr/share`。选择客户机扁平设备树后，共享 AxVM FDT 层必须把 `kernel.cmdline` 作为每虚拟机 `/chosen/bootargs` 覆盖，同时保留宿主启动参数作为后备；Axvisor 应用不得复制该策略。`qemu-*.toml` 中的 `uefi`、`to_bin`、加速、处理器功能和设备选择属于用例契约，axbuild 不得按目标体系结构或宿主 `/dev/kvm` 可用性推断或覆盖。Axvisor x86_64 运行时按 CPUID 选择 VMX 或 SVM，通用 QEMU 板卡与构建配置保持后端中立；持续集成保留 Intel/VMX 与 AMD/SVM 两个用例，但都不得选择 Cargo `vmx` 或 `svm` 功能，并使用同一后端中立客户机基线。
- **宿主扁平设备树处理器解析**：仍需更新客户机可见的物理处理器标识；只有 `phys_cpu_sets` 缺失时才能合成单元素调度掩码，经过验证的显式亲和掩码始终优先。
- **AArch64 实体板客户机扁平设备树**：客户机设备树必须在客户机 GPA 描述其可见的模拟 GICv3、体系结构定时器、PL011 和 virtio-mmio 设备，不得把整棵 RK3588 宿主设备树当作这些设备已直通。配置的虚拟设备只使用 `id + model + typed options`；体系结构解析后的设备图拥有其内存映射输入输出范围和虚拟控制器输入，客户机固件产物必须匹配这些资源。不得恢复旧 `emu_devices`、裸客户机地址或 IRQ 字段、`interrupt_mode` 或物理 IRQ 直通配置捷径。每个 GPPT GICR 帧为 128 KiB，帧不能重叠，且只有最后一个客户机重分发器设置 `GICR_TYPER.Last`。PSCI `CPU_SUSPEND` 未支持时，移除逐处理器 `cpu-idle-states` 引用和 `/cpus/idle-states`。Linux 使用的模拟 PL011 必须接受字节、半字和字访问；实体对称多处理控制台记录必须共享宿主输出锁，避免客户机输出与 Axvisor 日志交错。
- **AArch64 设备图固件发布**：直接从 `DeviceModel::firmware()` 与 `ResolvedDeviceResources` 生成常规运行时 FDT 节点；Linux 和 Starry DTB 模板必须省略这些节点，同兼容字符串的模板节点应报错。Zephyr 等把资源编译进映像的客户机可以保留与确定性设备图槽一致的构建叠加层，但运行时 DTB 仍由设备图生成，并以契约测试核对两者。
- **原始 RTOS 客户机与 IVC 映像**：沿选定的 `image_location` 与加载器追踪 `kernel_path` 解析；转换或启动原始映像前，核验 ELF 入口和每个非空 `PT_LOAD` 都落在虚拟机 GPA 内存范围内。`MapAlloc(0)` 保持声明的客户机 GPA 固定，`MapIdentical(1)` 可能重定位客户机可见基址，两者不可互换。把控制器根文件系统显式传给 `cargo xtask axvisor qemu`，因为 axbuild 可能用受管默认值替换 QEMU 文件中的磁盘路径。把设备资源编译进映像的原始客户机必须匹配解析设备图的内存映射输入输出基址与大小、虚拟控制器中断标识、MAC、头模式和段。RTOS 启用内存管理单元时，通过其 ioremap 或设备 API 映射设备 GPA，不得直接解引用物理值；直接内存访问队列地址仍需独立论证客户机物理地址转换契约。原生 QEMU 启动只作为基线证据，不能当作 Axvisor 客户机或 IVC 证据。多路复用控制台解析身份与终止标记前，只规范化精确的前导 `[VM n] ` 标签。用 `debugfs` 向 ext4 映像注入文件时，写入前后运行 `e2fsck`，并确认 `stat` 输出含 inode，因为缺失路径时 debugfs 也可能成功退出。保留原始失败，并在每次修复后使用新的不可变证据目录。
...
- **AArch64 直通客户机中断所有权**：为异常级 2 宿主初始化的通用中断控制器处理器接口使用分离的优先级下降与停用模式，而直通客户机可只用 `EOIR` 完成物理中断。客户机首次运行前，把固定处理器接口交给客户机并切换到组合完成模式；保持宿主中断在精确保留 DAIF 的客户机进入与退出钩子之间屏蔽。模拟共享外设中断必须经过预分配的逐虚拟处理器所有权门：先取得全局通用中断控制器锁，再取得门，只路由非活动线路，并在恢复宿主中断状态前回收待处理状态；重入时保留活动线路路由。设备生产者不得直接挂起目标为客户机的物理共享外设中断，也不得在客户机拥有该接口时发送宿主处理器间中断。
- **AArch64 异常级 2 发布与向量所有权**：启用 `HCR_EL2.VM` 前发布当前异常级宿主回调，并在 HCR 更新前后执行 `ISB`。普通宿主 `VBAR_EL2` 始终归 `ax-cpu`；每次虚拟处理器进入只在世界切换窗口保存宿主向量并安装客户机退出向量，每个退出都在返回宿主 Rust 前恢复宿主向量，不得把虚拟处理器向量作为永久的逐处理器启用副作用。
...
- **AArch64 GIC 完成屏障**：写入 GICv3 `ICC_EOIR0_EL1`、`ICC_EOIR1_EL1` 或 `ICC_DIR_EL1` 后，必须在解除宿主 IRQ 屏蔽、返回无关的固定处理器宿主任务或依赖电平重采样前执行 `ISB`。否则保留的 CNTV 活动状态可能阻止同优先级、不同 PPI 的宿主 CNTHP 调度时钟变为可见；不得通过在虚拟 GIC 退休前提前停用客户机 CNTV 令牌规避该停顿。
- **动态中断路由锁**：`somehal::irq::IRQ_ROUTES` 会从硬中断读取，因此读写都必须取得字段级保存中断状态的锁。不能用普通自旋获取；本地中断重入会递归同一登记表并使全部处理器停顿。
- **AArch64 分配物理共享外设中断**：通过硬件支持列表项投递经过所有权检查的宿主共享外设中断，客户机和物理中断标识相同。精确的客户机向量窗口和恢复宿主 `VBAR_EL2` 后进入的普通宿主向量都必须查询同一条预注册、无分配路由。普通宿主入口只在执行优先级下降后分离路由所有的共享外设中断，通过固定 AxVM 接收端发布，并把 `DIR` 留给客户机退休；如果发布与路由移除竞争，中断栈上的 RAII 声明必须保留宿主后备完成责任。正常退休让物理 GIC 停用，不得在软件回收后重复宿主停用。客户机陷阱停用或销毁时显式停用；该操作完成电平重采样。旧列表项尚未回收时到达新的确认，应保留到补充阶段创建下一硬件列表项。未分配的 GICv3 中断与低优先级中断仍归宿主，通过已注册消息信号中断叶路由解析父控制器并保留完整 24 位中断标识；这不扩大已验证共享外设中断之外的直通范围。
...
9. 协作式调度器下的客户机对称多处理应在有效亲和掩码允许时，把新建虚拟处理器任务放到不同的初始宿主运行队列。客户机硬件处理器标识与稠密宿主调度标识保持分离，为后续迁移保留完整任务亲和性，并在远端处理器激活任务前发布虚拟机运行时记账。激活时必须重新核验运行时处理器上界和当前亲和性，不匹配时返回显式错误，并回滚已发布的虚拟处理器与运行时生命周期状态。
10. 客户机 `CPU_ON` 退出处理期间，调用方虚拟处理器仍登记为当前宿主处理器上的活动虚拟处理器。只在目标虚拟处理器的 `Starting` 保留状态下配置其已保存、未激活的后端状态；目标已发布的宿主任务首次在选定处理器运行前，不得把目标安装为当前虚拟处理器或进行绑定。PSCI 要求该操作异步：激活前发布目标任务及其虚拟机生命周期参与状态，激活被接受后立即返回成功，并让 `Starting` 在目标任务绑定前保持可观测的 `ON_PENDING`。不得在调用方虚拟处理器不可抢占的运行边界等待目标发布；目标侧取消或绑定失败必须同时回滚虚拟处理器状态和生命周期参与状态。

- **解析设备图固件发布**：只在机器所有的控制器、定时器和串口拓扑安装后发布常规 FDT 节点。机器补丁不得检查已配置模型名称，也不得预先创建设备图所有的节点。唯一默认中断控制器缺少固件 phandle 时，必须先分配 phandle，再发布 `interrupt-parent`。
