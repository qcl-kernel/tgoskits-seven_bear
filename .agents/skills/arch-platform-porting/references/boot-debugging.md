...
AArch64 常规虚拟设备 FDT 节点只在通用控制器、定时器和串口补丁完成后，使用各设备图模型的 `DeviceFirmwareSpec` 与节点的 `ResolvedDeviceResources` 发布。Linux 或 Starry DTB 模板若已含同一常规模型，说明模板陈旧，应明确失败而不是生成两个设备。Zephyr 等从构建叠加层编译驱动资源的客户机，应比较编译后的内存映射输入输出与共享外设中断元组和确定性设备图槽，并确认运行时 DTB 含设备图生成的节点。
...
- 启用 `HCR_EL2.VM` 前发布当前异常级宿主中断回调，并在 HCR 更新前后执行 `ISB`。虚拟处理器执行窗口外始终安装普通宿主 `VBAR_EL2`；客户机进入时保存它，只在世界切换窗口安装虚拟处理器向量，并在宿主 Rust 运行前由每次客户机退出恢复。否则异常可能观察到从未设计为同时生效的向量、回调和控制寄存器组合。当前异常级同步异常的内核恐慌报告至少包含 `ESR_EL2`、`FAR_EL2`、`ELR_EL2`、`SPSR_EL2` 和 `HCR_EL2`，以便把偶发宿主地址转换错误追溯到第一次无效访问，而不是只根据综合征分类。
