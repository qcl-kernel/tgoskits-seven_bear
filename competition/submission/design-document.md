...
本方案在 Orange Pi 5 Plus 的 RK3588 单芯片上运行 AxVisor、至少 2 vCPU 的 StarryOS 客户机与 RTOS 客户机。StarryOS 承担 USB 摄像头采集、神经网络推理与控制决策，RTOS 负责确定性动作执行与状态回传，主数据通道为 IVC/1 over UDP/IPv4。本文同时给出实时化、网络协议、隔离、AI 闭环、物理执行器和可复现证据的设计边界。

版本基线：目标分支基于 upstream PR #2182 通过 CI 时对应的 `upstream/dev` 提交 `f70cf8d0eadc43caf176e7873244e8fae8154d9c`；材料重建输入 HEAD 为 `0118ed5bbda46fefca64a39424a270e20232dff8`。冻结的正式 RT 数据归属于 `c82da8464ab69e7da95e9be08293559e67b28fac`，并非该输入 HEAD 的板端重跑。
...
![System architecture](assets/charts/01-system-architecture.png)
...
![RT worst of runs](assets/charts/02-rt-worst-of-runs.png)
...
![RT p99 pairs](assets/charts/03-rt-p99-pairs.png)
...
![Vision optimization](assets/charts/06-vision-optimization.png)
...
![SO-100 ID1](assets/charts/09-so100-id1.png)
