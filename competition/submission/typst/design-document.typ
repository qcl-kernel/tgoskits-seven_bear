...
#show: ilm.with(
  ...
  abstract: [本方案在 Orange Pi 5 Plus 的 RK3588 单芯片上运行 AxVisor，承载至少 2 vCPU 的 StarryOS 客户机与 RTOS 客户机。StarryOS 负责 USB 摄像头采集、神经网络推理与控制决策，RTOS 负责确定性动作执行与状态回传，主数据通道为 IVC/1 over UDP/IPv4。本文同步给出实时化、网络协议、隔离、AI 闭环、物理执行器和可复现证据的设计边界。],
