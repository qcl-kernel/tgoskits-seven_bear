...
| 早期 RT 单对诊断 | **不作为正式门** | C-RT 单对 `m2_exit_gate_met=false`；保留为发现尾延迟问题的诊断证据 |
| 当前源码 RT 性能门 M2 | **PASS** | C-RT-F；五配对、每指标每 run 10,000 样本、shared/partitioned 双 soak ≥1,800 秒，`m2_exit_gate_met=true` |
...
### 3.1 早期 shared/partitioned 单对冒烟
...
idle guest + 受控 host interference treatment，不外推为 WCET 或所有压力模型都改善。
23 个 compact 文件位于
