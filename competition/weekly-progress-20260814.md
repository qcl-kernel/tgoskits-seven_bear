...
端点、三客户机动态网络隔离用例和确定性证据归档工具。

一句话总结：本周完成三项赛题主链路的证据收口，将 RT-Thread、FreeRTOS 从原生
实时基线升级为可运行的 AxVisor Guest/IVC 端点，并以四组可靠性活动和三客户机
隔离实跑补齐多 RTOS 与网络负例证据。
...
说明：2026-08-13 的当前源码 RT 冒烟仅为一组 100 样本 shared/partitioned 对照，
periodic jitter 和 dispatch p99 未达到非退化门，且不满足五配对矩阵，因此机器判定
`m2_exit_gate_met=false`。本周不将该单对结果表述为“当前源码实时性提升”。
...
| 赛题方向 | 当前状态 | 本周可交付证据 |
| --- | --- | --- |
| 实时 RTOS 化改造 | 已完成核心改造和多核客户机部署 | RT 配置、采集/比较脚本、实体冒烟、历史五配对与 soak 数据 |
...
按现有评分映射的证据自评，主评分材料约可支撑 `98/100`，并可主张 StarryOS 替代
Linux 的 `+4` 与多 RTOS 的 `+2`，合计加分 `+6`；该数字为内部证据自审，不代表
官方评审结果。

## 当前风险与下周计划

1. 从最终交付 commit 重跑 RT 受控干扰五配对和双侧 soak，将性能结论与最终源码
   完整绑定；当前源码单对 RT 冒烟不能替代此项。
