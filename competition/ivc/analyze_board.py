...
AXVISOR_VM_CONSOLE_PREFIX = re.compile(r"\[VM [0-9]+\][ \t]*")
...
def analyze(
    ...
    *,
    ...
    text = GUEST_CONSOLE_PREFIX.sub("\n", text)
    lines = AXVISOR_VM_CONSOLE_PREFIX.sub("\n", text).splitlines()
...
def parse_rtos(
    ...
    combined_result = find_optional_record(
        lines,
        RTOS_RESULT_PREFIX,
    ...
    messages = select_rtos_record(
        find_optional_record(lines, RTOS_MESSAGES_PREFIX, message_fields),
        combined_result,
        message_fields,
...
def select_rtos_record(
    compact_record: dict[str, str] | None,
    combined_result: dict[str, str] | None,
    required_fields: tuple[str, ...],
    compact_prefix: str,
) -> dict[str, str]:
    typed_combined_result = (
        combined_result
        if combined_result is not None
        and rtos_fields_are_well_typed(combined_result, required_fields)
        else None
    )
    if compact_record is None:
        if typed_combined_result is None:
            raise AnalysisError(f"missing complete {compact_prefix.strip()} record")
        return typed_combined_result
    if typed_combined_result is not None and any(
        compact_record[field] != typed_combined_result[field]
        for field in required_fields
    ):
        raise AnalysisError(
            f"conflicting complete {compact_prefix.strip()} and "
            f"{RTOS_RESULT_PREFIX.strip()} records"
        )
    return compact_record


def rtos_fields_are_well_typed(
    record: dict[str, str], required_fields: tuple[str, ...]
) -> bool:
    try:
        for field in required_fields:
            if field == "profile":
                if required(record, field, RTOS_RESULT_PREFIX) not in RUN_PROFILES:
                    return False
            else:
                integer(record, field, RTOS_RESULT_PREFIX)
    except AnalysisError:
        return False
    return True
