...
class OrtControlContractTests(unittest.TestCase):
    ...
    def test_smoke_config_runs_starry_ort_control_with_the_zephyr_peer(self) -> None:
        ...
        with STARRY_CONFIG.open("rb") as source:
        ...
        self.assertEqual(starry["devices"]["passthrough"], [])
        self.assertEqual(starry["devices"]["disabled"], [])
        self.assertEqual(
            starry["devices"]["virtual"],
            [
                {
                    "id": "disk0",
                    "model": "virtio-blk-mmio",
                    "image_path": "/home/orangepi/axvisor-guest/starry-ivc-rootfs-ort-control-smoke.img",
                },
                {
                    "id": "net0",
    ...
    def test_full_config_runs_1800_sample_ort_control_without_npu_handoff(self) -> None:
        ...
        with STARRY_FULL_CONFIG.open("rb") as source:
            starry = tomllib.load(source)
        ...
        disk = next(
            device
            for device in starry["devices"]["virtual"]
            if device["id"] == "disk0"
        )
        self.assertEqual(disk["model"], "virtio-blk-mmio")
        self.assertEqual(
            disk["image_path"],
            "/home/orangepi/axvisor-guest/starry-ivc-rootfs-ort-control.img",
        )
