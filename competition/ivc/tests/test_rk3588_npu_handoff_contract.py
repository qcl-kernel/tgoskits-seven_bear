...
class Rk3588NpuHandoffContractTests(unittest.TestCase):
    ...
    def test_axvisor_propagates_the_handoff_capability(self) -> None:
    ...
    def test_only_npu_core_registers_are_passed_to_the_guest(self) -> None:
        ...
        self.assertEqual(devices["passthrough"], [{"path": "/npu@fdab0000"}])
        self.assertEqual(devices["disabled"], [])
        self.assertNotIn("interrupt_mode", devices)
        self.assertNotIn("passthrough_devices", devices)
        self.assertNotIn("passthrough_addresses", devices)
        self.assertEqual(
            devices["virtual"],
            [
                {
                    ...
                    "image_path": "/home/orangepi/axvisor-guest/starry-rknpu-rootfs-smoke.img",
                }
            ],
        )
        self.assertEqual(
        ...
        for base_gpa, length in NPU_CORE_REGIONS:
            self.assertLess(base_gpa, 1 << 32)
    ...
    def test_guest_dts_leaves_conventional_devices_to_the_resolved_graph(self) -> None:
        ...
        self.assertNotIn("virtio_mmio@", source)
        self.assertNotIn('compatible = "virtio,mmio";', source)
        self.assertIn("AxVM adds configured conventional devices", source)
    ...
    def test_rknpu_control_backend_reuses_the_starry_zephyr_full_loop(self) -> None:
        ...
        self.assertEqual(
            guest_config["devices"]["passthrough"],
            [{"path": "/npu@fdab0000"}],
        )
        self.assertEqual(guest_config["devices"]["disabled"], [])
        self.assertEqual(
            [(device["id"], device["model"]) for device in guest_config["devices"]["virtual"]],
            [
