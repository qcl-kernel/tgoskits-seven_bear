...
static void report_combined_result(const struct ivc_server *server, const char *profile)
{
...
#if CONFIG_IVC_EXIT_AFTER_EXPECTED_COMMANDS
static void report_poweroff_evidence(const struct ivc_server *server, const char *profile)
{
	uint32_t copy;

	/* The controller emits its terminal metrics on the shared UART immediately
	 * after the final ACK. Let those records drain before replaying both the
	 * combined result and poweroff identity in this quiet window.
	 */
...
#if CONFIG_IVC_EXIT_AFTER_EXPECTED_COMMANDS
	report_poweroff_evidence(server, profile);
	sys_poweroff();
#endif
}
