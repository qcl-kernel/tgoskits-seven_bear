...
fn do_rt_snapshot_sync(cmd: &ParsedCommand) {
    ...
    let request = rt_snapshot_request();
...
#[cfg(all(feature = "fs", feature = "rt-trace"))]
fn rt_snapshot_request() -> SnapshotRequest<'static> {
    SnapshotRequest {
        vm_id: 1,
        backing_index: 0,
...
mod tests {
    ...
    #[test]
    #[cfg(all(feature = "fs", feature = "rt-trace"))]
    fn rt_snapshot_alias_targets_the_approved_result() {
        let request = rt_snapshot_request();

        assert_eq!(request.vm_id, 1);
        assert_eq!(request.backing_index, 0);
        assert_eq!(request.output_path, "/home/rt");
    }
}
